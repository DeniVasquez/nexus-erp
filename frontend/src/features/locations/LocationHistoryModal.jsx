...
function LocationHistoryModal(props) {
  const [history] = createResource(
    () => props.location,
    async (location) => {
      if (!location) return null;
      return locationsApi.getHistory(location._id);
    },
  );
        ...
        <div class="flex-1 overflow-y-auto p-6">
          ...
          <Show when={history() && history().data}>
            <Show
              ...
              fallback={
                <div class="text-center py-8 text-gray-500 dark:text-gray-400">
                  No hay cambios registrados para esta ubicación
                </div>
              }
            >
              <div class="space-y-4">
                      ...
                      <div class="flex justify-between items-start mb-3">
                        <div>
                          <p class="text-sm font-medium text-gray-900 dark:text-white">
                            {log.action === "create" && "✨ Ubicación creada"}
                            {log.action === "update" &&
                              "✏️ Ubicación actualizada"}
                          </p>
