...
function CompanyHistoryModal(props) {
  const [history] = createResource(
    () => props.company,
    async (company) => {
      if (!company) return null;
      return companiesApi.getHistory(company._id);
    },
  );
        ...
        <div class="flex justify-between items-center px-6 py-4 border-b border-gray-200 dark:border-gray-800">
          <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
              {props.company?.name} ({props.company?.nit})
            </p>
          </div>
          <button
                      ...
                      <div class="flex justify-between items-start mb-3">
                        <div>
                          <p class="text-sm font-medium text-gray-900 dark:text-white">
                            {log.action === "create" && "✨ Empresa creada"}
                            {log.action === "update" &&
                              "✏️ Empresa actualizada"}
                          </p>
